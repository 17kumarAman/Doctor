...
//             return res.status(400).json({ error: 'Doctor not available on this date' });
//         }
...
//                 apptTime <= schedule.break_end)
//         ) {
//             return res.status(400).json({ error: 'Appointment time outside doctor availability hours' });
//         }

//         // Optional: Check if the appointment slot is already booked or occupied
...
//         const query = `INSERT INTO appointments (${fields.join(', ')}) VALUES (${placeholders})`;
...
//         // Optional: Add more filters like date, status, etc.
...
//         const allowedStatus = ['Pending', 'Accepted', 'Rejected', 'Confirmed', 'Cancelled'];
       ...
       WHERE doctor_id = ? AND available_date = ?`,
        ...
        if (hourlyCount[0].count >= 4) {
            return res.status(400).json({
                error: 'Maximum 4 appointments per hour limit reached for this time slot'
            });
        }
...
export const getAvailableTimeSlots = async (req, res) => {
    try {
        ...
        if (scheduleRows.length === 0) {
            return res.status(404).json({ error: 'Doctor not available on this date' });
        }
        ...
        const breakStart = schedule.break_start ? new Date(`1970-01-01T${schedule.break_start}`) : null;
        const breakEnd = schedule.break_end ? new Date(`1970-01-01T${schedule.break_end}`) : null;
        ...
        const availableCount = availableSlots.filter(slot => slot.status === 'available').length;
        const bookedCount = availableSlots.filter(slot => slot.status === 'booked').length;
        const breakCount = availableSlots.filter(slot => slot.status === 'break').length;
        const unavailableCount = availableSlots.filter(slot => slot.status === 'unavailable').length;
...
export const getAllAppointmentsWithDoctor = async (req, res) => {
    try {
        ...
        let countQuery = 'SELECT COUNT(*) as total FROM appointments a JOIN doctors d ON a.doctor_id = d.id';
        const countParams = [];
