...
const {
  ...
  status,
    ...
    // Agar image bheji hai to upload karein
    ...
    // Body + image dono ka data collect karo
    const updateData = removeUndefined({
      ...
      available_days,
      available_time,
      profile_image: profileImageUrl, // Image ka URL yahan include
      status,
    });
    ...
    // Agar body empty hai aur image bhi nahi aayi to error
...
export const getAllDoctors = async (req, res) => {
  try {
    const [rows] = await db.execute(
      `SELECT id, full_name, email, phone, gender, dob, specialization, qualification, experience_years, bio, consultation_fee, available_days, available_time, profile_image, status, created_by, created_at, updated_at FROM doctors`
    );
